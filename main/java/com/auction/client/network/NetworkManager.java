...
public class NetworkManager {
    ...
    public boolean send(NetworkMessage message) {
        ...
        if (sessionToken != null && message.getSenderId() == null) {
            message.setSenderId(sessionToken);
        }
        try {
            synchronized (out) {
    ...
    public boolean send(MessageType type, Object payload) {
        return send(new NetworkMessage(type, payload));
    }
    ...
    private void handleDisconnection() {
        ...
        MessageBus.getInstance().publish(
            new NetworkMessage(MessageType.ERROR, "Connection lost with server"));
    }
